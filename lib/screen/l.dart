...
class LPage extends StatefulWidget {
  ...
  LPage({required this.doctorName, required this.doctorId});
...
class _LPageState extends State<LPage> {
  ...
  Future<void> fetchData() async {
    ...
    print('Hospital ID from SharedPreferences: $hospitalId');
    ...
    return Scaffold(
        ...
        body: Column(
          children: <Widget>[
            Card(
              child: Container(
                width: double.infinity, // Set the width to occupy the full width of the card
                child: ListTile(
                  title: Text(scheduleData.isNotEmpty
                      ? scheduleData[0]['doctor']['name']
                      : ''),
                  subtitle: Text('' + (scheduleData.isNotEmpty
                                  ...
                                  child: ElevatedButton(
                                    onPressed: () async {
                                      if (scheduleItem != null && scheduleItem['book_appointment'] != null && !scheduleItem['book_appointment']) {
                                        ...
                                        print('Selected Schedule ID: ${scheduleItem['_id']}');

                                        // Store the selected schedule ID in SharedPreferences
                                        SharedPreferences prefs = await SharedPreferences.getInstance();
                                        prefs.setString('selectedScheduleId', scheduleItem['_id']);
                                      }
                                    },
  ...
  Future<String?> getHospitalIdFromSharedPreferences() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    return prefs.getString('hospitall_id');
  }
